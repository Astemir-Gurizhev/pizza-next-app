	...
	return (
		<>
			...
			<div
				ref={ref}
				className={cn(
					'flex rounded-2xl flex-1 justify-between mx-11  relative h-11 z-30',
					className
				)}
			>
					...
					>
						{products.map(product => (
							<Link
								onClick={onClickItem}
								key={product.id}
								className='flex items-center gap-3 px-3 py-2 hover:bg-orange-500/10'
								href={`/product/${product.id}`}
							>
								<img
									className='rounded-sm h-10 w-8'
